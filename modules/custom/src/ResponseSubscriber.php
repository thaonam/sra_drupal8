...
function alterResponse(FilterResponseEvent $event) {
  ...
  if ($node instanceof Node && $node->getType() == 'announcement' && !$node->isPublished() && $this->currentUser->isAuthenticated()) {
    drupal_set_message('This show has now expired.', 'warning');
    $event->setResponse(new RedirectResponse('notification'));
  }
  // }
}
